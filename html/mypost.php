...
<?php 
 ...
 <?php 
		...
		$sql = "SELECT user.userid, profile.name, post.* FROM post JOIN user ON post.userid = user.userid JOIN profile ON user.username = profile.username where  ltrim(rtrim(content)) != '' and post.userid = '$userid' ORDER BY post.datepost DESC, post.postid DESC ";
